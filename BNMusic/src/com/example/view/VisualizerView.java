...
public class VisualizerView extends View{
    ...
    private byte[] myBytesWave;
    private byte[] myBytesFft;
    private byte[] myOldBytesTop;
    private byte[] myOldBytesBottom;
    private float[] myPoints;
    private float[] myFftPoints = new float[4];
    private boolean visualizerMode;
    ...
    public VisualizerView(Context context,AttributeSet attrs)
    {
     ...
     private void initView() {
		// TODO Auto-generated method stub
             ...
             paintRect.setStrokeWidth(2f);
             paintRect.setAntiAlias(true);
             paint.setStyle(Style.FILL_AND_STROKE);
             int colorFrom = Color.RED;
             int colorTo = Color.YELLOW;
             LinearGradient lg = new LinearGradient(paddingLeft, paddingTop, contentWidth+paddingLeft, contentHeight+paddingTop, colorFrom, colorTo, TileMode.CLAMP);
	         paint.setShader(lg);
     }
        ...
        protected void onDraw(Canvas canvas) {
        	...
        	if(visualizerMode)
        	{
        		...
        		for(int i = 0 ; i < myBytesWave.length-1 ; i++)
        		{
        			...
        			myPoints[i*4+1] = viewHeight/2+((byte)(myBytesWave[i]+128))*(contentHeight/2)/128;
        			myPoints[i*4+2] = paddingLeft+contentWidth*(i+1)/(myBytesWave.length-1);
        			myPoints[i*4+3] = viewHeight/2 +((byte)(myBytesWave[i+1]+128))*(contentHeight/2)/128;
        		}
